    ...
    <div className="flex-1 flex flex-col min-h-0 bg-transparent font-outfit">
      {cart.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-gray-500 opacity-60 py-20">
          <div className="w-16 h-16 rounded-2xl bg-cyan-500/5 border border-cyan-500/10 flex items-center justify-center text-cyan-500/30 drop-shadow-[0_0_15px_rgba(0,240,255,0.2)] mb-4 animate-pulse">
              ...
              {cart.map(item => (
                <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                  <td className="p-5 pl-6">
                    ...
                    <div className="text-[10px] text-cyan-500/70 font-mono tracking-wider mt-0.5">{item.barcode}</div>
                  </td>
                  <td className="p-5 text-sm">
                    <span className="px-2.5 py-1 bg-black/40 border border-white/10 rounded-lg text-[10px] font-black uppercase tracking-wider text-cyan-300">
                      {item.category}
                    </span>
                  </td>
                  ...
                  <td className="p-5 text-center">
                    <div className="inline-flex items-center gap-2.5 bg-black/40 rounded-xl p-1 border border-white/5 shadow-inner">
                      ...
                      <button 
                        onClick={() => onUpdateQty(item.id, 1)}
                        className="w-7 h-7 flex items-center justify-center rounded-lg bg-cyan-500/20 hover:bg-cyan-500/40 text-cyan-300 transition-colors"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
