...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  List<Sale> _filterSales(List<Sale> sales) {
    ...
    // Apply search query with multi-token deep search across all transaction fields & items
    final q = _searchQuery.trim().toLowerCase();
    if (q.isNotEmpty) {
      final tokens = q.split(RegExp(r'\s+'));
      filtered = filtered.where((sale) {
        final itemsSearchable = sale.items?.map((i) => '${i.productName} ${i.productBarcode ?? ''} ${i.productCategory ?? ''}').join(' ') ?? '';
        final searchable = '${sale.id} ${sale.total.toStringAsFixed(2)} ${sale.total.toStringAsFixed(0)} ${sale.paymentMethod} ${sale.userName ?? ''} ${sale.customerName ?? ''} ${sale.customerPhone ?? ''} ${sale.storeBranch ?? ''} $itemsSearchable'.toLowerCase();
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by ID, item, customer, cashier...',
